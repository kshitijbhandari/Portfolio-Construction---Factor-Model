...
with st.sidebar:
    ...
    if is_recommended:
        ...
        strategy_choice = st.selectbox(
            ...
            help="Strategy 1 = best betas, Strategy 2 = mean betas, Strategy 3 = median betas",
        )
        beta_source_map = {"Strategy 1": "best", "Strategy 2": "mean", "Strategy 3": "median"}
...
with tab1:
    ...
    if is_recommended:
        ...
        _excel_path = next((p for p in _excel_candidates if os.path.exists(p)), "beta_search_log.xlsx")
...
The optimizer targets those betas automatically.
        ...
        if not os.path.exists(_excel_path):
            st.warning("⚠️ `beta_search_log.xlsx` not found. Run the Optuna beta search first (model.ipynb).")
...
- Strategy 1: Best betas from Optuna
- Strategy 2: Mean of betas across trials
- Strategy 3: Median of betas across trials
