...
with st.sidebar:
    ...
    if is_recommended:
        ...
        st.divider()
        use_old_regime_data = st.checkbox(
            "Use original regime classification (pre-correction)",
            value=False,
            help=(
                "Regime-Adaptive Rotation uses a macro regime classifier (Investment "
                "Clock). By default this now uses a corrected, uniformly-classified "
                "version of that data (2011-2025 on one consistent source). Check "
                "this to instead use the original classification, which mixed two "
                "data sources at a 2024 seam. Only affects the Regime-Adaptive "
                "strategy -- the others are unaffected either way."
            ),
        )
        beta_log_filename = "beta_search_log.xlsx" if use_old_regime_data else "beta_search_log_full_reclassified.xlsx"
        ...
        beta_log_filename = "beta_search_log_full_reclassified.xlsx"  # not used in customized mode
...
# ============================================================================
if is_recommended:
    tab1, tab2, tab_detail, tab7, tab_attr, tab3, tab5 = st.tabs([
        "📊 Run Backtest", "📈 Results", "🔄 Portfolio Detail",
        "Strategy Comparison", "🎯 Performance Attribution",
        "🔍 Risk Analysis", "ℹ️ Info",
    ])
    tab4 = tab6 = None
else:
    tab4, tab6, tab1, tab2, tab_detail, tab7, tab_attr, tab3, tab5 = st.tabs([
...
The optimizer targets those betas automatically.
        ...
        _local_candidates = [
            os.path.join(data_dir, beta_log_filename),
            beta_log_filename,
            os.path.join(os.path.dirname(__file__), beta_log_filename),
            os.path.join(os.path.dirname(__file__), "data", beta_log_filename),
        ]
        _local_path = next((p for p in _local_candidates if os.path.exists(p)), None)

        if _local_path:
            _excel_source = _local_path
            st.success(f"✅ `{beta_log_filename}` found locally.")
        else:
            st.warning(f"⚠️ `{beta_log_filename}` not found. Upload it below.")
            _uploaded = st.file_uploader(
                f"Upload {beta_log_filename}",
                type=["xlsx"],
...
with tab_attr:
    ...
    st.caption(
        "Decomposes the Regime-Adaptive strategy's realized excess return into "
        "market / size / value factor contributions plus alpha, broken down by "
        "macro regime. Always uses the **original regime classification** "
        "(`beta_search_log.xlsx`) regardless of the setting in the sidebar -- "
        "this is the data behind the results already reported for this strategy, "
        "not the newer corrected/reclassified version."
    )

    _attr_excel_candidates = [
    ...
    _attr_local_path = next((p for p in _attr_excel_candidates if os.path.exists(p)), None)

    if _attr_local_path:
        _attr_excel_source = _attr_local_path
        st.success("✅ `beta_search_log.xlsx` (original classification) found locally.")
    else:
        st.warning("⚠️ `beta_search_log.xlsx` not found. Upload it below.")
        _attr_excel_source = st.file_uploader(
            "Upload beta_search_log.xlsx", type=["xlsx"], key="attr_upload",
        )

    st.info(f"""
**Configuration used for attribution:**
- Period: {oos_start} for {oos_months} months, rebalance every {rebalance_every} months
- Lookback: {lookback_months} months | Portfolio: {K_max} stocks, max {w_max*100:.0f}% per position
- Beta source: **regime_mean** (Regime-Adaptive Rotation, forced for this tab)
    """)

    if st.button("▶️ Run Performance Attribution", use_container_width=True, type="primary"):
        if _attr_excel_source is None:
            st.error("Upload `beta_search_log.xlsx` before running.")
        else:
            progress_status = st.empty()

            def _report_progress_attr(rb, i, total):
                progress_status.text(f"📅 Rebalancing {i}/{total} — currently at {rb.strftime('%Y-%m')}")

            with st.spinner("⏳ Running Regime-Adaptive backtest for attribution..."):
                try:
                    from utils import run_recommended_backtest
                    attrib_res = run_recommended_backtest(
                        stock_returns_data=stock_returns_data,
                        fama_french_data=fama_french_data,
                        index_returns=index_returns_data,
                        oos_start=oos_start,
                        oos_months=oos_months,
                        beta_source="regime_mean",
                        rebalance_every=rebalance_every,
                        risk_aversion=risk_aversion,
                        lookback_months=lookback_months,
                        K_max=K_max,
                        ...
                        turnover_cap=turnover_cap,
                        initial_capital=initial_capital,
                        excel_path=_attr_excel_source,
                        R_full_prebuilt=R_full,
                        ticker_universe_prebuilt=ticker_universe,
                        threshold_mode="dynamic",
                        progress_callback=_report_progress_attr,
                    )
                    progress_status.empty()
                    st.session_state.attrib_result = attrib_res
                    st.success("✅ Attribution backtest completed!")
                except Exception as e:
                    progress_status.empty()
                    st.error(f"❌ Attribution backtest failed: {str(e)}")
                    st.exception(e)

    if "attrib_result" in st.session_state:
        attrib_res = st.session_state.attrib_result

        _ff_attr = fama_french_data.copy()
        _ff_attr["Date"] = pd.to_datetime(_ff_attr["Date"]) + pd.offsets.MonthEnd(0)
        _ff_attr = _ff_attr.set_index("Date").sort_index()
        _rf_attr = pd.to_numeric(_ff_attr["RF"], errors="coerce")
        if _rf_attr.abs().median(skipna=True) > 0.05:
            _rf_attr = _rf_attr / 100.0

        rebalance_log = attrib_res["rebalance_log"]
        rb_dates_sorted = sorted(pd.Timestamp(d) for d in rebalance_log.keys())

        def _active_rb(m):
            active = rb_dates_sorted[0]
            for rb in rb_dates_sorted:
                if rb <= m:
                    active = rb
                else:
                    break
        ...
        strat_ret = attrib_res["strategy_returns"]
        rows = []
        for m in strat_ret.index:
            rb = _active_rb(m)
            info = rebalance_log[str(rb.date())]
            achieved = info["achieved_betas"]
            regime = info.get("regime", "UNKNOWN")

            mf_ret = float(_ff_attr.loc[m, "MF"]) if m in _ff_attr.index else float("nan")
            smb_ret = float(_ff_attr.loc[m, "SMB"]) if m in _ff_attr.index else float("nan")
            hml_ret = float(_ff_attr.loc[m, "HML"]) if m in _ff_attr.index else float("nan")
            rf_ret = float(_rf_attr.loc[m]) if m in _rf_attr.index else 0.0

            mkt_contrib = achieved["MF"] * mf_ret
            smb_contrib = achieved["SMB"] * smb_ret
            hml_contrib = achieved["HML"] * hml_ret
            excess_ret = float(strat_ret.loc[m]) - rf_ret
            # Alpha = whatever excess return isn't explained by realized exposure
            # x realized factor return that month (selection/optimization skill
            # beyond the target-beta tilt itself).
            alpha_contrib = excess_ret - (mkt_contrib + smb_contrib + hml_contrib)

            rows.append({
                "Date": m,
                "Regime": regime,
                "Excess Return": excess_ret,
                "Market Contribution": mkt_contrib,
                "Size Contribution": smb_contrib,
                "Value Contribution": hml_contrib,
                "Alpha": alpha_contrib,
            })

        attrib_df = pd.DataFrame(rows).set_index("Date")
        COMPONENT_COLS = ["Market Contribution", "Size Contribution", "Value Contribution", "Alpha"]

        # Arithmetic (simple) annualization, not geometric compounding -- this is
        # what makes the four components sum exactly back to the total, since
        # mean() is linear. The headline "Ann. Return" elsewhere in this app uses
        # geometric compounding, so it won't match this total exactly; that gap
        # is expected and standard in return attribution, not an error.
        arith_total_ann = attrib_df["Excess Return"].mean() * 12
        component_ann = attrib_df[COMPONENT_COLS].mean() * 12

        st.divider()
        st.subheader("Overall Attribution (arithmetic-annualized)")
        cols = st.columns(5)
        cols[0].metric("Total Excess Return", f"{arith_total_ann:+.2%}")
        for c, name in zip(cols[1:], COMPONENT_COLS):
            val = component_ann[name]
            pct = (val / arith_total_ann * 100) if arith_total_ann != 0 else float("nan")
            c.metric(name, f"{val:+.2%}", f"{pct:+.1f}% of total")

        st.divider()
        st.subheader("Attribution by Regime")
        by_regime = attrib_df.groupby("Regime")[COMPONENT_COLS].mean() * 12
        by_regime["Months"] = attrib_df.groupby("Regime").size()
        st.dataframe(by_regime.round(4), use_container_width=True)

        st.divider()
        st.subheader("Illustrative Cumulative Contribution by Component")
        st.caption(
            "Each component compounded independently for visualization; won't sum "
            "exactly to the strategy's actual cumulative return (that requires "
            "geometric linking, e.g. Menchero/Carino smoothing) -- standard "
            "practice for attribution charts, not an error."
        )
        fig, ax = plt.subplots(figsize=(12, 6))
        cum_components = (1 + attrib_df[COMPONENT_COLS]).cumprod() - 1
        for col in cum_components.columns:
            ax.plot(cum_components.index, cum_components[col] * 100, label=col, linewidth=1.8)
        ...
        with st.expander("Monthly attribution detail"):
            st.dataframe(attrib_df.round(4), use_container_width=True)
    ...
    if not is_recommended:
        ...
        if _local_path_cmp:
            ...
            _excel_source_cmp = st.file_uploader(
                f"Upload {beta_log_filename} for comparison",
                type=["xlsx"],
